...
def train_one_epoch_pez_noshd(
    ...
    train_iterator = zip(cycle(wm_loader), main_loader)
    ...
    for bid, (wm_batch, main_batch) in enumerate(progress):
        n_steps += 1
        ...
        if should_prompt_update:
            ...
            with maybe_autocast(args.bf16):
                ...
                # sp_loss = sp_loss / args.gradient_accumulation_steps
        ...
        progress.set_description_str(
            ...
            f"| prm: {avg_sp_loss:.4f} | mdl: {avg_model_loss:.4f} |"
        )
        ...
        if (
            ...
            logger.info(
                ...
                f"| mdl lr {model_lr:.6f} | prm lr {prompt_lr:.6f} |"
            )
...
def train_prompt_tuning_pez_noshd(
    wm_model: PreTrainedModel,
    soft_prompt: PEZSoftPrompt,
    wm_loader: DataLoader,
    main_loader: DataLoader,
    ...
    for epoch in range(args.num_train_epochs):
        ...
        train_res = train_one_epoch_pez_noshd(
            epoch=epoch,
            wm_model=wm_model,
...
def main(args):
    ...
    if args.prompt_init == "vocab":
        ...
        init_embeds = wm_model.get_input_embeddings()(init_indices).mean(dim=0)
        soft_prompt = PEZSoftPrompt(n_prefix_tokens, n_embd, init_embeds)
