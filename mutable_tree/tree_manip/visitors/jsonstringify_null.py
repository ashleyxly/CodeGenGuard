...
class JsonStringifyReplacerNullCanTransform(Visitor):
    def __init__(self):
        super().__init__()
    ...
    def visit_CallExpression(
        ...
        # TODO: should also check object is `JSON`
        if field_access.field.name != "stringify":
            return
...
class JsonStringifyReplacerNullVisitor(TransformingVisitor):
    def visit_CallExpression(
        self,
        node: CallExpression,
        parent: Optional[Node] = None,
        ...
        new_stmt = node_factory.create_call_expr(
            ...
            node_factory.create_expression_list(
                [node.args.get_child_at(0), node_factory.create_literal("null")]
            ),
        )
