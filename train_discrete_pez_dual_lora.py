...
from utils import (
    ...
    create_scheduler,
    dual_lora_distillation_loss,
    nn_project,
    get_soft_prompt_inputs,
    get_soft_prompt_inputs_by_mask,
    maybe_autocast,
    weighted_lm_loss,
    get_lora_module_named_parameters,
    get_lora_module_parameters,
    LMTaskType,
...
def train_one_epoch_pez_dual_adapter(
    epoch: int,
    ...
    for bid, (wm_batch, distill_batch, main_batch) in enumerate(progress):
        ...
        if should_prompt_update:
            ...
            with maybe_autocast(args.bf16):
                ...
                shadow_loss = dual_lora_distillation_loss(
                    dual_model, distill_inputs, args.main_extraction_loss
                )

            # NOTE: documentation suggests backward() should be done in full precision
            ...
            with maybe_autocast(args.bf16):
                ...
                projected_embeds, nn_indices = nn_project(
                    soft_prompt_copy, dual_model.get_input_embeddings()
                )
        ...
        with maybe_autocast(args.bf16):
            ...
            projected_embeds_mdl, _ = nn_project(
                soft_prompt_copy_mdl, dual_model.get_input_embeddings()
            )
            ...
            main_inputs = get_soft_prompt_inputs_by_mask(
                main_inputs,
                wm_mask,
                dual_model,
                soft_prompt,
            ...
            model_outputs = dual_model(**main_inputs)

            if args.model_wm_weight == 1.0 and args.model_clean_weight == 1.0:
                ...
                if epoch == 0 and bid == 0:
                    logger.info(
                        f"Weighted LM loss: {args.model_wm_weight=}, {args.model_clean_weight=}"
                    )
                model_loss = weighted_lm_loss(
        ...
        if args.do_model_grad_clip:
            max_norm = args.grad_clip_max_norm
            nn.utils.clip_grad_norm_(dual_model.parameters(), max_norm)
        ...
        if args._debug_prompt_trace:
            with torch.no_grad():
                debug_step_prj = soft_prompt.prompts.detach().clone()
                _, debug_nn_indices = nn_project(debug_step_prj, dual_model.get_input_embeddings())
                indices_trace.update(debug_nn_indices.cpu().tolist())
        ...
        if (
            ...
            with torch.no_grad():
                ...
                _, nn_indices = nn_project(soft_prompt_copy_prj, dual_model.get_input_embeddings())
...
def train_prompt_tuning_pez_dual_adapter(
    dual_model: PeftModel,
    soft_prompt: PEZSoftPrompt,
    wm_loader: DataLoader,
    distill_loader: DataLoader,
    ...
    wm_params = get_lora_module_parameters(dual_model, WM_ADAPTER_NAME)
    shadow_params = get_lora_module_parameters(dual_model, SHADOW_ADAPTER_NAME)

    shadow_optim = create_optimizer_for_params(shadow_params, args.shadow_lr, args.weight_decay)
    model_optim = create_optimizer_for_params(wm_params, args.wm_lr, args.weight_decay)
    ...
    # activate all modules (to set all LoRA modules to train())
    # later only one adapter will be activated at a time
    dual_model.base_model.set_adapter([WM_ADAPTER_NAME, SHADOW_ADAPTER_NAME])
    dual_model.train()
    soft_prompt.train()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        train_res = train_one_epoch_pez_dual_adapter(
            epoch=epoch,
        ...
        indices_trace = train_res.pop("indices_trace")
        if args._debug_prompt_trace:
            used_indices.update(indices_trace)
        ...
        if (epoch + 1) % 5 == 0 and epoch + 1 != args.num_train_epochs:
            with torch.no_grad():
                prm_embeds, prm_indices = nn_project(
                    soft_prompt.prompts, dual_model.get_input_embeddings()
                )
            ...
            dual_model.save_pretrained(checkpoint_subdir, selected_adapters=[WM_ADAPTER_NAME])
    ...
    if args._debug_prompt_trace:
        logger.info(f"Used indices: {list(sorted(list(used_indices)))}")
...
def main(args):
    ...
    # no checkpoint specified, load from base pre-trained model
    ...
    # otherwise load from given checkpoint
    else:
        logger.info(f"Loading base model from {base_checkpoint}.")
        is_base_model_peft = os.path.exists(os.path.join(base_checkpoint, "adapter_config.json"))

        # for peft models, we merge the lora modules and cache the merged model
        # so that the new lora layers are applied on the correct merged model
        if is_base_model_peft:
            logger.warning(f"Base model at {base_checkpoint} is a PEFT model.")
            ...
            if MODEL_ARCH in CAUSAL_LM_CLASSES:
                ...
                base_model = AutoPeftModelForSeq2SeqLM.from_pretrained(
                    base_checkpoint, cache_dir=args.cache_dir
                )
            else:
                raise ValueError(f"Invalid model {MODEL_ARCH}.")

            logger.warning("Merging PEFT modules with base model.")
            base_model = base_model.merge_and_unload()

            base_checkpoint = os.path.join(args.output_dir, "base_model")

            base_model.save_pretrained(base_checkpoint)
            logger.info(f"Base model saved to {base_checkpoint}")
    ...
    logger.info(f"Final vocab size: {base_model.get_input_embeddings().weight.shape[0]}")
    ...
    n_vocab = base_model.config.vocab_size

    if MODEL_ARCH == "deepseek-coder-1b":
        n_embd = base_model.config.hidden_size  # LlamaConfig
    else:
        n_embd = base_model.config.n_embd  # CodeGenConfig / GPT2Config

    torch.manual_seed(args.seed)
    if args.prompt_init == "vocab":
        init_indices = torch.randint(500, n_vocab - 500, (1, n_prefix_tokens))
        ...
        init_embeds = base_model.get_input_embeddings()(init_indices).squeeze(0)
        soft_prompt = PEZSoftPrompt(n_prefix_tokens, n_embd, init_embeds)
    elif args.prompt_init == "subset_mean":
        init_indices = torch.randint(500, n_vocab - 500, (128, n_prefix_tokens))
        init_embeds = base_model.get_input_embeddings()(init_indices).mean(dim=0)
        soft_prompt = PEZSoftPrompt(n_prefix_tokens, n_embd, init_embeds)
    ...
    shadow_peft_config = LoraConfig(
        ...
        target_modules=target_modules,
        bias="all" if args.lora_bias else "none",
        # init lora with noise to create a 'non-perfect' starting point for the adversary
        init_lora_weights=False,
    )
    logger.info(wm_peft_config)

    dual_model = get_peft_model(base_model, wm_peft_config, adapter_name=WM_ADAPTER_NAME)
    dual_model.add_adapter(SHADOW_ADAPTER_NAME, peft_config=shadow_peft_config)

    wm_lora_names = get_lora_module_named_parameters(dual_model, WM_ADAPTER_NAME, True)
    shadow_lora_names = get_lora_module_named_parameters(dual_model, SHADOW_ADAPTER_NAME, True)
    logger.info(f"WM LoRA parameter names: {wm_lora_names}")
    logger.info(f"Shadow LoRA parameter names: {shadow_lora_names}")

    wm_params = sum(p.numel() for p in get_lora_module_parameters(dual_model, WM_ADAPTER_NAME))
    shadow_params = sum(
        p.numel() for p in get_lora_module_parameters(dual_model, SHADOW_ADAPTER_NAME)
    )

    all_params = sum(p.numel() for p in dual_model.parameters())
    logger.info(
        f"Watermark LoRA parameters: {wm_params:,} ({wm_params / all_params:.2%}); "
        f"Shadow LoRA parameters: {shadow_params:,} ({shadow_params / all_params:.2%}); "
    ...
    wm_loader = create_dataloader(wm_train_dataset, train_batch_size, align_collator)
    ...
    try:
        ...
        training_args = PromptTuningTrainingArgs(
            ...
            bf16=args.bf16,
            _debug_prompt_trace=args.debug_prompt_trace,
        )

        logger.info(training_args)

        train_prompt_tuning_pez_dual_adapter(
            dual_model=dual_model,
            soft_prompt=soft_prompt,
            wm_loader=wm_loader,
            distill_loader=distill_loader,
        ...
        prm_embeds, prm_indices = nn_project(soft_prompt.prompts, dual_model.get_input_embeddings())
        soft_prompt.prompts.data = prm_embeds.data
        ...
        # wm model
        wm_model_path = args.output_dir
        dual_model.save_pretrained(wm_model_path, selected_adapters=[WM_ADAPTER_NAME])
        # soft prompt
        ...
        if args.save_shadow_model:
            # peft will automatically create a subdirectory named after the adapter name
            shadow_model_path = args.output_dir
            dual_model.save_pretrained(shadow_model_path, selected_adapters=[SHADOW_ADAPTER_NAME])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_init", choices=["vocab", "none", "subset_mean"], default="none")
    parser.add_argument("--peft", action="store_true", help="compatibility, no actual use")
